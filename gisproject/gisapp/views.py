...
from .models import UserProfile  # adjust if your model is elsewhere
...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Give "Can view user profile" permission
            ...
            messages.success(request, f'Account created for {user.username}, marked as staff, and permission granted!')
            return redirect('profile')
        else:
            messages.error(request, 'There was an error in registration.')
    else:
